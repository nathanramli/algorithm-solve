...
int main(){
    ...
    // Basically we add every suffixes and substract it by the lcp
    // We can just calculate it by n(n + 1)/2 which equal to sum of every suffixes
    // And then substract by the sum of lcp
    ll len = n-1;
    cout << len*(len + 1)/2 - lcp_sum << "\n";
}