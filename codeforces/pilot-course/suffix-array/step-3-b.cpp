...
string s;
int find_upper_bound(int low, int high, vector<int> &p, string &target, int target_len){
    int n = s.size();
    int ans = -1;
    while (low <= high)
    {
        ...
        for(int i = 0; i < target_len; i++){
            if(p[mid] + i >= n || (int)target[i] < (int)s[p[mid] + i]){
                high = mid - 1;
                found = false;
                break;
            }
            else if((int)target[i] > (int)s[p[mid] + i]){
                low = mid + 1;
                found = false;
                break;
            }
...
int find_lower_bound(int low, int high, vector<int> &p, string &target, int target_len){
    int ans = -1;
    int n = s.size();
    while (low <= high)
    {
...
int main(){
    ...
    for(int i = 0; i < qn; i++){
        ...
        int upper_bound = find_upper_bound(0, n - 1, p, queries[i], len);
        int lower_bound = find_lower_bound(0, n - 1, p, queries[i], len);
